Shader "Custom/Toon Lit Atten -TS3" {
	Properties {
	...
	SubShader {
		...
		inline half4 LightingToonRamp (SurfaceOutputCustom s, half3 lightDir, half atten)
		{
			...
			half3 ramp = tex2D (_Ramp, float2(d, s.screenUV.y)).rgb;

			// Adapted from https://stackoverflow.com/questions/596216/formula-to-determine-brightness-of-rgb-color
			// Y = 0.375 R + 0.5 G + 0.125 B
			// float luma = ramp.r * 0.475 + ramp.g * 0.5 + ramp.b * 0.235;

			// luma += (tex2D(_ShadeNoise, float2(luma ,s.meshUV.y * _ShadeNoiseScale))).r;

			// if(luma < _ShadowMaskThreshold){
			// 	luma = (tex2D(_ShadowTex, s.screenUV)).rgb;
			// 	//luma = (tex2D(_ShadowTex, s.meshUV)).rgb;
			// }else luma = 1;
